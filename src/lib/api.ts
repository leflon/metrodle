...
export async function getRandomStation(enabledTypes: Record<string, boolean>): Promise<string> {
	...
	const typeString = Object.entries(enabledTypes).reduce((acc, [index,val]) => acc + (val ? index + ',' : ''), '');
...
export async function sendGame(guesses: Guess[], correct: string, action: 'finished' | 'reset' | 'leave'): Promise<void> {
